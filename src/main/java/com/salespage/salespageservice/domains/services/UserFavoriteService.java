...
public class UserFavoriteService extends BaseService{
    public void createAndUpdateUserFavorite(String username, String refId, FavoriteType type, Float star, Boolean isLike) throws Exception {
        ...
        if(type.equals(FavoriteType.PRODUCT)){
            if(!productStorage.isExistByProductId(refId)) throw new ResourceNotFoundException("Không tồn tại sản phẩm này");
        }

        if(type.equals(FavoriteType.STORE)){
            if(!sellerStoreStorage.isExistByStoreId(refId)) throw new ResourceNotFoundException("Không tồn tại cửa hàng này");
        }
        ...
        if(type.equals(FavoriteType.SELLER)){
            ...
            if(!accountStorage.existByUsernameAndRole(favoriteUser.getUsername(), UserRole.SELLER)) throw new ResourceNotFoundException("Người dùng không hợp lệ");
        }

        if(type.equals(FavoriteType.SHIPPER)){
            User favoriteUser = userStorage.findUserById(refId);
            if(Objects.isNull(favoriteUser)) throw new ResourceNotFoundException("Không tồn tại người bán này này");
            if(!accountStorage.existByUsernameAndRole(favoriteUser.getUsername(), UserRole.SHIPPER)) throw new ResourceNotFoundException("Người dùng không hợp lệ");
        }
